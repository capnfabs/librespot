...
use librespot_core::{session::Session, keymaster::get_token};
use log::{info, error};
...
async fn refresh_token(sess: &Session) -> Option<RspotifyToken> {
    ...
    let client_id = env::var("LIBRESPOT_CLIENT_ID").unwrap();
    let token_result = get_token(sess, &client_id, SCOPE).await;
    match token_result {
        Ok(token) => {
            let expires_in = Duration::seconds(token.expires_in.into());
            ...
            rspot_tok.expires_in = expires_in;
            info!("Got token");
            Some(rspot_tok)
        }
        Err(e) => {
